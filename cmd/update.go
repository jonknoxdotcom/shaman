...
func init() {
	...
	updateCmd.Flags().IntVarP(&cli_anon, "anonymous", "a", 0, "Anonymisation level 0..5")
...
func upd(args []string) {
	...
	for scanner.Scan() {
		...
		if trip_name < ssf_name {
			for trip_name < ssf_name {
				...
				writeRecord(w, amWriting, cli_anon, verbosity, "N", "", trip_modt, trip_size, trip_name, "")
		...
		if trip_name == ssf_name {
			...
			if ssf_modtime == trip_modt && ssf_length == trip_size && !cli_rehash {
				...
				writeRecord(w, amWriting, cli_anon, verbosity, "U", ssf_shab64, trip_modt, trip_size, ssf_name, "")
			} else {
				...
				if flag != "" {
					// changed
					writeRecord(w, amWriting, cli_anon, verbosity, "C", sha_b64, trip_modt, trip_size, ssf_name, flag)
				} else {
					// verified and unchanged
					writeRecord(w, amWriting, cli_anon, verbosity, "V", sha_b64, trip_modt, trip_size, ssf_name, flag)
				}
			}
		...
		if ssf_name != "" && trip_name > ssf_name {
			writeRecord(w, amWriting, cli_anon, verbosity, "D", "", "", "", ssf_name, "") // verified unchanged
		}
	}
	...
	for trip_name != "" {
		writeRecord(w, amWriting, cli_anon, verbosity, "N", "", trip_modt, trip_size, trip_name, "") // new
