...
func upd(args []string) {
	...
	for scanner.Scan() {
		...
		///fmt.Println("2: " + trip_name + " < " + ssf_name + " ?")
		if trip_name < ssf_name {
			for trip_name < ssf_name {
				// write record, lazy hash (generated by writer if needed)
				writeRecord(w, amWriting, verbosity, "N", "", trip_modt, trip_size, trip_name, "")
		...
		if trip_name == ssf_name {
			...
			if ssf_modtime == trip_modt && ssf_length == trip_size && !cli_rehash {
				...
				if flag != "" {
					...
					writeRecord(w, amWriting, verbosity, "C", sha_b64, trip_modt, trip_size, ssf_name, flag) //  changed
					//nchg++
				} else {
					// verified and unchanged
					writeRecord(w, amWriting, verbosity, "V", sha_b64, trip_modt, trip_size, ssf_name, flag) // verified unchanged
				}
	...
	for trip_name != "" {
		...
		writeRecord(w, amWriting, verbosity, "N", "", trip_modt, trip_size, trip_name, "") // new
		//
		// nnew++
