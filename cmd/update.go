	...
	for scanner.Scan() {
		...
		if trip_name == ssf_name {
			...
			if ssf_modtime == trip_modt && ssf_length == trip_size && !cli_rehash {
				...
				writeRecord(w, amWriting, verbosity, "U", ssf_shab64, trip_modt, trip_size, ssf_name, "")
			} else {
				...
				if flag != "" {
					// changed
					writeRecord(w, amWriting, verbosity, "C", sha_b64, trip_modt, trip_size, ssf_name, flag)
				} else {
					// verified and unchanged
					writeRecord(w, amWriting, verbosity, "V", sha_b64, trip_modt, trip_size, ssf_name, flag)
				}
			}
